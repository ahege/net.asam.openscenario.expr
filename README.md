...
*	Expressions that contains number and floating point literals like 
    - ${-15 + 3.14 + 23 + 2.1E-7 + sqrt(9) + floor(pow(3.2,3))}
*	Expressions that further contain parameter values like 
    - ${$defaultWidth +3} 
*	Expressions with +,-,*,% operators or basic functions like 
    - ${$defaultWidth + 3/2}
    - ${4*3 -$counter} 
...
* Operators are exclusively supported for numerical value datatypes. For int, unsignedInt, unsignedShort and double datatypes. Not for boolean, string or dateTime datatypes.
...
OpenSCENARIO may process safety critical data which must guarantee accurate calculation. Like in javascript, every numeric value is converted safely to a 64 byte double value.
...
The internal datatypes must ensure, that arithmetic overflow is avoided. This may most efficiently and safely achieved when using 64-byte double type for all internal calculations. Any OpenSCENARIO type like int, unsingedInt, unsignedShort values are converted into 64 byte double values for internal calculations.
...
E.g. If 64 byte double values are used for internal calculation the expression of the next example must abort with an error, since the limit for 64 byte double values is exceeded.

```
${-1.7976931348623157E308 -1}
```
E.g. with an error : ```"Internal Overflow (limits of internal 64 byte double value exceeded)"```

In this way, any internal arithmetic overflow is detected.

## Converting result
Though internal calculation exclusively uses the 64 byte double datatype, the expression must be converted into the expected type the sceanrio file expects.
Converting the result of an expression to an integer value type (int, unsingedInt, unsignedShort) should fail if there is any information loss like cutting non-zero digits after the decimal separator.

E.g. the expression in this example must issue an error.

```xml
<ParameterDeclaration parameterType="unsignedInt" name="numberOfExecutions" value="${34+3.45}"/>
```
Use `ceil`, `round`, or `floor` for valid conversion.

```xml
<ParameterDeclaration parameterType="unsignedInt" name="numberOfExecutions" value="${floor((34+3.45))}"/>
```
